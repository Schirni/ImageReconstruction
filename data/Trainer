...
from Model import GeneratorAB, GeneratorBA, Discriminator, NoiseEstimator, DiscriminatorMode
...
class Trainer(nn.Module):
    def __init__(self, input_dim_a, input_dim_b, upsampling=0, noise_dim=16, n_filters=64,
        ...
        logging.info("Shuffle Channels:   %s" % str(shuffle))
    ...
    def generator_update(self, x_a, x_b):
        ...
        # translate 1
        if self.shuffle == True: # Shuffle channels
            r = torch.randperm(x_a.shape[1])
            x_a_shuffle = x_a[:, r]
            x_ab = self.gen_ab(x_a_shuffle)
            ...
            r = torch.randperm(x_ba.shape[1])
            x_ba_shuffle = x_ba[:, r]
            x_bab = self.gen_ab(x_ba_shuffle)
    ...
    def validate(self, x_a, x_b):
        ...
        with torch.no_grad():
            ...
            self.valid_loss_ssim_a = self.loss_ssim_a(x_ba)
            self.valid_loss_ssim_b = self.loss_ssim_b(x_ab)
    ...
    def startBasicTraining(self, base_dir, ds_A, ds_B, ds_valid_A=None, ds_valid_B=None,
                           ...
                           iterations=int(10e5), num_workers=8, validation_history=False, batch_size=1):
        self.cuda()
        start_it = self.resume(base_dir)
        # Init Callbacks
        from Callback import HistoryCallback, ProgressCallback, SaveCallback, PlotAB, RandomPlotAB, PlotBAB, PlotABA, ValidationHistoryCallback
        ...
        # start update cycle
        for it in tqdm(range(start_it, iterations)):
            self.train()
