...
class DiscriminatorMode(Enum):
    ...
    RANDOM = 'RANDOM' # use a single discriminator for a random channel and one for the combined channels
...
class GeneratorBA(nn.Module):
    def __init__(self, input_dim, output_dim, noise_dim, depth, depth_noise, n_downsample, dim=64, output_activ='tanh', skip_connections=True, **kwargs):
        ...
        self.noise_blocks = nn.Sequential(*self.noise_blocks)  # * unpack everything from list and put it in sequential
        ##################### Core #####################
...
class Discriminator(nn.Module):
    def __init__(self, input_dim, n_filters, num_scales=3, depth_discriminator=4,
        ...
        assert discriminator_mode in list(DiscriminatorMode), 'Discriminator mode not in the defined List!'
        ...
        self.random_discs = nn.ModuleList() if discriminator_mode == DiscriminatorMode.RANDOM else None
        ...
        if discriminator_mode == DiscriminatorMode.SINGLE_PER_CHANNEL:
            ...
            for i in range(input_dim):
                self.channel_discs['%d' % i] = channel_disc
        if discriminator_mode == DiscriminatorMode.RANDOM:
            for _ in range(num_scales):
                self.random_discs.append(self._make_net(1, n_filters))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_scales):
            ...
            rand_idx = np.random.randint(x.shape[1]) # select random integer from channels (100)
            random_x = x[:, rand_idx:rand_idx+1] # shape: [batch=1, 1, height, width]
            outputs.append(self.random_discs[i](random_x))
            #scale_disc = self.random_discs[i]
            #disc_pred = scale_disc(random_x)
            #outputs.append(disc_pred)
            x = self.downsample(x)
        return outputs
